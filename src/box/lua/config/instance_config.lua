...
return schema.new('instance_config', schema.record({
    ...
    metrics = schema.record({
        ...
        --
        -- Keep the enum variant in addition to the plain string scalar so the
        -- generated JSON Schema exposes built-in metric groups as suggestions,
        -- while still allowing custom metric selectors.
        include = schema.array({
            items = schema.union({
                ...
                discriminator = discriminators['metrics.include'],
            }),
            validate = validators['metrics.include'],
        }),
        exclude = schema.array({
            items = schema.union({
                ...
                discriminator = discriminators['metrics.exclude'],
            }),
            validate = validators['metrics.exclude'],
        }),
        labels = schema.map({
